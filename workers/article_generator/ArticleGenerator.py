...
class ArticleGenerator:
    ...
    def _process_single_document(self, doc, chunk_results: dict, lock: threading.Lock) -> None:
        ...
        try:
            ...
            content_source = 'stored'  # Track where content came from
            ...
            if article_url:
                ...
                try:
                    if fetched_content:
                        ...
                        # Write with utf-8, small safety trim to avoid extremely large writes
            ...
            article_metadata = {
                ...
                'total_score': total_score,
                'doc_id': doc_id,
                'fetched_content': fetched_content,  # Store fetched content for saving
                'content_source': content_source,  # Track content source
            }
            ...
            try:
                ...
                update_payload = {
                    ...
                    'updated_at': datetime.now(timezone.utc).isoformat(),
                    'fetched_content': article_metadata.get('fetched_content'),  # Save fetched content (short form)
                    'fetched_full_text': article_metadata.get('fetched_content'),  # persisted full fetched text for auditing
                    'content_source': article_metadata.get('content_source'),  # Track content source
                }
