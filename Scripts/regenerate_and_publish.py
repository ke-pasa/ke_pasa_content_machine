...
def main(article_id: str):
    ...
    # Stage 4: generate initial Telegram preview
    tg_result = translator._stage4_telegram(stage2, metadata)
    if not tg_result or not isinstance(tg_result, dict):
        logger.error('Translator returned no tg_preview (stage4) for %s', article_id)
        return 3
    ...
    if not tg_preview:
        logger.error('No tg_preview generated by stage4 for %s', article_id)
        return 4

    # Stage 5: final refinement (if translator exposes it)
    try:
        if hasattr(translator, '_stage5_finalize'):
            s5 = translator._stage5_finalize(tg_result, metadata)
            if isinstance(s5, dict) and s5.get('tg_preview'):
                final_preview = s5.get('tg_preview')
                final_flags = s5.get('flags') or tg_flags
            else:
                final_preview = tg_preview
                final_flags = tg_flags
        else:
            final_preview = tg_preview
            final_flags = tg_flags
    except Exception as e:
        logger.exception('Stage5 finalization failed for %s: %s', article_id, e)
        final_preview = tg_preview
        final_flags = tg_flags

    # Save both stage4 and stage5 results back to articles_ru (do not touch updated_at)
    update_payload = {
        'stages': {
            'telegram': {'tg_preview': tg_preview, 'flags': tg_flags},
            'telegram_final': {'tg_preview': final_preview, 'flags': final_flags}
        },
        'telegram_preview': final_preview,
        'telegram_flags': final_flags,
    }
